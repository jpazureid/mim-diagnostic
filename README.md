...
`Test-NetConnection` の `TcpTestSucceeded` が `False`、または `Test-WSMan` が失敗する場合、FIMSynchronizationService サーバー側の情報を取得できません。その場合は、通常診断で `-FIMServiceOnly` オプション、または `-FIMSyncOnly` オプションを使用し、FIMService サーバーの情報と FIMSynchronizationService サーバーの情報を、それぞれ分けて取得する必要があります。
...
FIMSynchronizationService へのリモート接続ができない環境の場合は `-FIMServiceOnly` を指定します。また、その場合は、FIMSynchronizationService サーバー側でも `-FIMSyncOnly` を指定してスクリプトを実行することで診断情報を取得できます。
...
FIMSynchronizationService サーバー上で実行し、特定の AD オブジェクトについて、AD / Connector Space / Metaverse の情報を取得します。
...
既定では、指定した `-Logpath` 配下に `MIMLOG_yyyyMMdd_HHmmss.zip` を出力します。

`-FIMServiceOnly` を指定した場合は、指定した `-Logpath` 配下に `MIMLOG_FIMSERVICE_yyyyMMdd_HHmmss.zip` を出力します。

`-FIMSyncOnly` を指定した場合は、指定した `-Logpath` 配下に `MIMLOG_FIMSYNC_yyyyMMdd_HHmmss.zip` を出力します。
